...
### A share for the content

`YARG-share.cfg` in this directory is the share definition — song library,
playlists and album art in one place, rather than song content living in
`appdata` where a service keeps its state.

```bash
cp YARG-share.cfg /boot/config/shares/YARG.cfg
mkdir -p /mnt/user/YARG/songs /mnt/user/YARG/playlists /mnt/user/YARG/art
chown -R nobody:users /mnt/user/YARG
find /mnt/user/YARG -type d -exec chmod 0775 {} + ; find /mnt/user/YARG -type f -exec chmod 0664 {} +
```

**That is not enough to export it over SMB, and everything will look like it
worked.** emhttpd picks the share up immediately — it appears in
`/usr/local/emhttp/state/shares.ini` with every setting correct — but
`/etc/samba/smb-shares.conf` is generated by emhttpd rather than by `rc.samba`,
and it does not regenerate until a share command comes through. Until then the
share is configured, real, and invisible to every Windows machine.

Trigger it with the same command the SMB security form posts, using the values
already in the cfg so it is a no-op to the file:
...
smbclient -L localhost -N | grep -i YARG      # the far-end proof, not the file
```

Adjust `shareWriteList` to your own user. Then drop songs in at
`\\<server>\YARG\songs` and restart the container — it re-indexes on start and
never writes to that directory.
...
**The songs mount is the exception to the "new services go on `/mnt/cache/...`"
rule.** That rule exists because of SQLite-on-FUSE corruption and a historically
full array — neither of which applies to read-only song files, and charts are
hand-made work worth parity protection. So `/songs` comes from the array-backed
`YARG` share while `/data`, which is service state, stays on cache.

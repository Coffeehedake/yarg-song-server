...
// What a player with a very large library actually does to /api/v1/have, and
// where the ceiling really is.
//
// This matters because the CLIENT decides the body size, not the server: both
// yarg-sync and the in-game mirror POST every chart hash they already hold, so
// the request grows with the player's library and nobody had ever measured what
// that costs or when it stops working. A limit discovered by a user with 200,000
// songs is a sync that fails with no explanation.
//
// Note this deliberately does NOT assert a duration. A timing threshold in CI is
// a flake generator, and the number that matters here is the size ceiling, which
// is exact. The cost is logged so a future regression has something to read.
func TestHaveScalesToALargeClientLibraryAndRefusesAnAbsurdOne(t *testing.T) {
	srv, _ := newTestServer(t, defaultSpecs())

	// 40 hex characters, distinct, shaped exactly like a real chart hash.
	hashes := func(n int) []string {
		out := make([]string, n)
		for i := range out {
			out[i] = fmt.Sprintf("%040x", i)
		}
		return out
	}

	post := func(hs []string) (int, int) {
		t.Helper()
		body, err := json.Marshal(haveRequest{ChartHashes: hs})
		if err != nil {
			t.Fatal(err)
		}
		start := time.Now()
		...
		if err != nil {
			t.Fatalf("%d hashes (%d bytes): transport error: %v", len(hs), len(body), err)
		}
		defer resp.Body.Close()
		io.Copy(io.Discard, resp.Body)
		t.Logf("%7d hashes = %8d bytes -> %d in %v", len(hs), len(body), resp.StatusCode,
			time.Since(start).Round(time.Millisecond))
		return resp.StatusCode, len(body)
	}

	// Realistic and then some. 31,109 is the largest library this project has
	// ever indexed; 100,000 is the extrapolation SCALE.md reasons about.
	for _, n := range []int{1_000, 10_000, 31_109, 100_000} {
		code, _ := post(hashes(n))
		if code != http.StatusOK {
			t.Errorf("a client holding %d songs got %d; a real library reaches this size", n, code)
		}
	}

	// Past the 8 MB cap the answer must be a clean 413, not a 500, a truncated
	// read silently treated as "this client has nothing", or a hang. Being told
	// you are missing your entire library is the worst possible failure here:
	// it would re-download everything.
